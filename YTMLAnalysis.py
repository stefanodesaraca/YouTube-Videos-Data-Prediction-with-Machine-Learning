import YTAPIFile
from YTDataFormatConversion import YTDataConversion #To extract the fileName calling the method of the class
import pandas as pd
from scipy import stats #For Shapiro-Wilk Normality Test
...
class YouTubeML:
    def __init__(self, channelTitle):
        self.fileName = None #Assigned later in the program
        self.apiKey = YTAPIFile.KEY
        self.channelID = YTAPIFile.ID
        self.EDAPlotsPath = f"./YouTubeML/{channelTitle}/YouTubeMLEDA/YTPlots/"
        self.shapiroWilkPlotsPath = f"./YouTubeML/{channelTitle}/YouTubeMLEDA/YTShapiroWilkPlots/"
        self.modelsPath = f"./YouTubeML/{channelTitle}/YTModels/"
        self.TestScorePlotsPath = f"./YouTubeML/{channelTitle}/YTModelsTestScoreSummaryPlots/"
        self.testReportPath = f"./YouTubeML/{channelTitle}/YTModelsReport/"
        self.modelsPredictionPlotsPath = f"./YouTubeML/{channelTitle}/YTModelsPredictionPlots/"
        self.EDAYTDF = None #YouTube EDA DataFrame available to all class functions
        self.YTDF = None #YouTube Data DataFrame for ML related functions
    ...
    def getCSVFileName(self):
        ...
        YTDC = YTDataConversion(self.apiKey, self.channelID)

        try:
            channelTitle = YTDC.findChannelTitle()
            fileName = f"./YouTubeML/{channelTitle}/{channelTitle}" + "CSV.csv"
    ...
    def setYTDFs(self):
        ...
        try:
            ...
            print("\033[91mFile not found, have you set the correct channel ID or converted the JSON file into CSV?\033[0m")
            exit()
    ...
    def setScaling(self, scalingLevelH):
        ...
        #In case of future maintenance this is to ensure that the structure of the setScaling function remains the same
        else:
    ...
    #Declaring ZScore as a static method means that it still is a method of the class, but it's not bound to the class instance (object), so it doesn't use the self parameter because it can't modify the class state
    @staticmethod
    ...
    def ShapiroWilkTest(self, targetFeatureName, data):
        ...
        return plotName, SWQQPlot, self.shapiroWilkPlotsPath
    ...
    def ExploratoryDataAnalysis(self):
        ...
        if len(self.EDAYTDF.index) > 450: #Leaving room for a good number of videos to analyze
            ...
            self.EDAYTDF = self.ZScore(self.EDAYTDF, ["totalSecondsDuration"])  #Executing it twice for better results, but this is optional
    ...
    def createExploratoryAnalysisPlots(self):
        ...
        def YTNumDataCorrPlot():
            ...
            return plotName, YTNumDataCorrGraph, self.EDAPlotsPath
        ...
        def YTDataLinePlot():
            ...
            return plotName, YTDataLinePlot, self.EDAPlotsPath
        ...
        def viewsDistributionPlot():
            ...
            #plt.tight_layout()
            return plotName, viewsDistributionGraph, self.EDAPlotsPath
        ...
        def secondsDurationAndLikesPlot():
            ...
            #plt.tight_layout()
            return plotName, secondsDurationAndLikesGraph, self.EDAPlotsPath
        ...
        def likeViewRatioScatterPlot():
            ...
            return plotName, viewLikeRatePlot, self.EDAPlotsPath
        ...
        def groupedFeaturesBarPlot():
            ...
            return plotName, fig, self.EDAPlotsPath
    ...
    def L1PenaltyFS(self, feature, targetColumn):
        ...
        if len(L1FS.get_feature_names_out()) == 0: #If not features were selected by model selection then:
            ...
            self.L1PredictionAndScores(feature, XTrainTLasso, XTestTLasso, YTrain, YTest)  #To not repeat myself twice I just incorporated the last part of the L1PenaltyFS Function in a separate one
            ...
            self.L1PredictionAndScores(feature, XTrainTLasso, XTestTLasso, YTrain, YTest)

            return L1FS.get_feature_names_out()


    @staticmethod
    def L1PredictionAndScores(feature, XTrainTLasso, XTestTLasso, YTrain, YTest):
    ...
    def getImportantFeaturesTTS(self, targetFeature): #Get Important Features Test-Train-Split
        ...
        targetFeatureIndependentVariablesList = list(self.FeaturesDict[targetFeature]) #This list contains specific predictors (returned by feature selection functions) for each target feature (at each call of this function the target feature, and so also its predictors, will change)

        #print("DataFrame Shape: ", self.YTDF.shape)
        #print("Columns in YTDF:", self.YTDF.columns)

        X = self.YTDF.loc[:, targetFeatureIndependentVariablesList]  # Subsetting the dataframe with only the important feature to predict the specific feature passed as formal parameter to the function
    ...
    def modelPredictionsPlot(self, targetFeature, model, XTest, YPredictions):
        ...
        return plotName, fig, self.modelsPredictionPlotsPath
    ...
    @savePlots
    def generateMLPlots(self, targetFeaturesMLDict):
        ...
        return FeaturesModelNames, ModelPlots, self.TestScorePlotsPath
    ...
    def executeMLAnalysis(self):
        ...
        self.generateMLPlots(self.targetFeaturesTestMLDict)
